Ext.define('mobileV1.view.SelectLocations', {
        ...
        config: {
			...
			items: [{
				...
				cls: "myTop",
				itemTpl: '<div style="height:70px"><img class="imageSetting" height="70" width="70" src="'+imageLocationPath+'{images}"><div style="padding-left:100px;margin-top:-70px;font-size:14px">{f_title}</br><div style="font-family:bold">Office Tel No: {f_tel_o}</div></div></div>',
				listeners: {
					itemtap: function(view, index, item, e){
						var tmp = Ext.JSON.decode(Ext.ComponentQuery.query('#locationhidden')[0].getValue());
						var rec = view.getStore().getAt(index);
						var cHttp = new XMLHttpRequest();
						var params = "content_id="+e.data.f_id+"&delete=0";
						...
						cHttp.onreadystatechange = function () {
							if (cHttp.readyState == 4 && cHttp.status == 200) {
								...
								Ext.getCmp('newDataView').setSrc(imagePath+tmpR[0].images);
							}
						}
						cHttp.send(params);
					}
