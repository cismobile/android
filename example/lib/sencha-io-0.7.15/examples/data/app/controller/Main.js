...
Ext.define('MyApp.controller.Main', {
    ...
    onChannles: function(channels){
      channels["map"].on("message", function(sender, message){
    ...
    getChannel: function(){
        return this.getApplication().sio.getChannel('map');
    },
      ...
      _updateLocation: function(user, geo) {
          ...
          if (record) {
              ...
              if(user.getData().email) {
                label = user.getData().email.split("@")[0];
              }
      ...
      syncLocation: function(publishAfterSync) {
          ...
          location.sync(function() {
              ...
              if (publishAfterSync === true) {
                  self.getChannel().publish({message:{channel: "updated"}}, function(){ console.log("publish complete", arguments);  });
              }
              self.updateMarkers();
          });
