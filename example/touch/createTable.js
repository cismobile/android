...
var getEaccStatement = function(id,cDate,eDay){
	var cHttp = new XMLHttpRequest();
	var params = "distributor_id="+id+"&cDate="+cDate+"&eDay="+eDay+"&type=eAccount";
	cHttp.open("POST", sysPath + '/api/Eaccount/EAccStatement', true);
	cHttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
	cHttp.setRequestHeader("Content-length", params.length);
	cHttp.setRequestHeader("Connection", "close");
	cHttp.onreadystatechange = function () {
		if (cHttp.readyState == 4 && cHttp.status == 200) {
			var tmpR = Ext.JSON.decode(cHttp.responseText);
			Ext.getStore('eAccStatement').setData(tmpR);
		}
	}
	cHttp.send(params);
}
...
var storeNews = function(){
	var cHttp = new XMLHttpRequest();
	var params = "company="+localStorage.companyid+"&type=news&deleted=0&fields=m.f_filename,cc.f_id,cc.f_title,cc.f_created_date,cc.f_description&date="+ Ext.Date.format(appDate,'Y-m-d');
...
var storeEvents = function(){
	var cHttp = new XMLHttpRequest();
	var params = "company="+localStorage.companyid+"&type=event&deleted=0&fields=m.f_filename,cc.f_id,cc.f_title,cc.f_created_date,cc.f_description&date="+ Ext.Date.format(appDate,'Y-m-d');
	cHttp.open("POST", sysPath + '/cms/CmsArticleSetup/ArticleNews', true);
	cHttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
	cHttp.setRequestHeader("Content-length", params.length);
	cHttp.setRequestHeader("Connection", "close");
	cHttp.onreadystatechange = function () {
		if (cHttp.readyState == 4 && cHttp.status == 200) {
			var tmpR = eval("(" + cHttp.responseText + ")");
			Ext.getStore('EventList').setData(tmpR);
		}
	}
	cHttp.send(params);
}
...
var storeMediaList = function(){
	var cHttp = new XMLHttpRequest();
	var params = "date="+ Ext.Date.format(appDate,'Y-m-d');
	cHttp.open("POST", sysPath + '/cms/CmsArticleSetup/MediaList', true);
	cHttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
	cHttp.setRequestHeader("Content-length", params.length);
	cHttp.setRequestHeader("Connection", "close");
	cHttp.onreadystatechange = function () {
		if (cHttp.readyState == 4 && cHttp.status == 200) {
			...
			Ext.getStore('storeMedia').setData(tmpR);
		}
	}
	cHttp.send(params);
}
...
var storeLocationSecStates = function(id,title){
	...
	cHttp.onreadystatechange = function () {
		if (cHttp.readyState == 4 && cHttp.status == 200) {
			...
			Ext.getStore('LocationSelectLocation').removeAll();
